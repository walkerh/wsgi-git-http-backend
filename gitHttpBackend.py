...
def run_git_http_backend(cgi_environ, input_string_io, log_std_err=False):
    ...
    environment and the file-like object input_string_io.

    See build_cgi_environ() and git documentation for the requirements
    for cgi_environ .

    input_string_io will normally be a StringIO object, but any object
    implementing the file protocol will work. Note that input_string_io
